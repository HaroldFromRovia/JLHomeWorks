...
public class VacationConsumer implements DocumentConsumer {
    ...
    public void consume(Channel channel, ChannelFactory factory) {
        ...
        DeliverCallback deliverCallback = (consumerTag, message) -> {
            ...
            pdfWriter.createTemplate(pdfWriter.createFile("vacation.pdf"))
                    .fillHeader("Ректору КФУ\n" +
                            "Гафурову И.Р.\n" +
                            "от ", user)
                    .fillBody("Заявление\n", TextAlignment.CENTER)
                    .fillBody("    Прошу освободить меня от посещения занятий. " +
                                    "Обязуюсь устранить долги по дисциплинам в установленные сроки. ",
                            TextAlignment.LEFT)
                    .fillFooter("Дата")
                    .fillFooter("Подпись")
                    .close();
